...
{
	output_hpp += '\t\tustring_t get_manifest_' + item + '() const { return "' + manifestCommonFinal[item] + '"_u; }\n';
}
...
output_hpp += '\n\n\t\tustring_t get_app_name(app_t app) const\n\t\t{\n\t\t\tswitch(app)\n\t\t\t{';
for(var proj in projectsManifests)
{
...
for ( i = 0; i < projectsToGen.length; i++ )
{
	...
	for(var item in projectsManifests[projectToGen])
	{
		...
		output_hpp += '\t\tustring_t get_app_' + projectToGen + '_manifest_' + item + '() const { return "' + projectsManifests[projectToGen][item] + '"_u; }\n';
	}
}
...
for(var i in possibleItems)
{
	...
	output_hpp += '\t\tustring_t get_common_manifest_' + item + '(app_t app) const\n\t\t{';
	for ( i = 0; i < projectsToGen.length; i++ )
	{
		var projectToGen = projectsToGen[i];
...
{
	output_hpp += '\t\tustring_t get_local_manifest_' + item + '() const { return "' + projectsManifests[local_app][item] + '"_u; }\n';
}

output_hpp += '\n\t\tapp_t get_local_app() const { return app_t::' + local_app + '; }\n';

output_hpp += '\n\n\t\tustring_t get_local_app_name() { return "' + local_app + '"_u; }';
