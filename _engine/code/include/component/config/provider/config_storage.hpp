...
#define ENGINE_CONFIG_LOCAL(type, name) \
		type get_local_##name(manifest_app_t::app_t app) const final { std::lock_guard<std::recursive_mutex> guard(mutex); return config_storage->retrieve(format_string(storage_settings->get()->key_for_local_##name(), app), settings->get()->local_##name()); } \
		set_result_t set_local_##name(manifest_app_t::app_t app, type val) final { std::lock_guard<std::recursive_mutex> guard(mutex); if(get_local_##name(app) != val) { config_storage->store(format_string(storage_settings->get()->key_for_local_##name(), app), val); return set_result_t::success; } else return set_result_t::no_change; }
