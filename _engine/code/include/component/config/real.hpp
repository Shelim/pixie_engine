...
#define ENGINE_CONFIG_LOCAL(type, name) \
		...
		virtual set_result_t set_local_##name(manifest_app_t::app_t app, type val) = 0;
#include "def/config.def"
	...
	public:
		...
		config_real_t(std::shared_ptr<manifest_app_t> manifest_app, std::unique_ptr<holder_t<config_t> > config_provider, std::unique_ptr<holder_t<config_changed_t> > config_changed_provider, std::shared_ptr<logger_t> logger) :
			config_t(manifest_app), messenger_config(messenger_config), config_provider(std::move(config_provider)), config_changed_provider(std::move(config_changed_provider)), logger(logger)
		{
			logger->log_msg(config, "Config component has started"_u);
		}

		~config_real_t()
		{
			logger->log_msg(config, "Config component has been disposed"_u);
		}
...
#define ENGINE_CONFIG_LOCAL(type, name) \
		type get_local_##name(manifest_app_t::app_t app) const final { std::lock_guard<std::recursive_mutex> guard(mutex); return config_provider->get_provider()->get_local_##name(app); } \
		void set_local_##name(manifest_app_t::app_t app, type val) final { std::lock_guard<std::recursive_mutex> guard(mutex); if(config_provider->get_provider()->set_local_##name(app, val) == config_provider_base_t::set_result_t::success) notify_on_change(app, config_local_t::name, to_string(val)); }
#include "def/config.def"
...
#include "component/config/provider/config_storage.hpp"
