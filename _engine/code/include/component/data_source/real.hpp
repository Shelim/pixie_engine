...
#include "utility/platform/default_output.hpp"
#include "component/data_source.hpp"
#include "utility/platform/scanners_provider.hpp"
...
namespace engine
{
	...
	public:
		...
		data_source_real_actual_t(std::shared_ptr<logger_t> logger, std::shared_ptr<data::scanners_provider_t> scanners_provider, std::shared_ptr<messenger_t> messenger, std::shared_ptr<data_archives_t> data_archives, std::shared_ptr<data_output_default_t> data_output_default) : logger(logger), scanners_provider(scanners_provider), messenger(messenger), data_output_default(data_output_default)
		{
			auto task = logger->log_task_start(data, "Initializing database providers engine"_u);
			flags.set_flag(flag_t::requested_rescan, true);
		...
		std::unique_ptr<data::output_t> get_output(const virtual_path_t & filename)
		{
			...
			data::provider_t * provider = get_state()->get_provider(filename);
			if (!provider)
				return std::move(data_output_default->construct_default_output(filename));
	...
	private:
		...
		{
			if (!flags.is_flag(flag_t::requested_rescan))
			{
		...
		std::shared_ptr<data::scanners_provider_t> scanners_provider;
		std::shared_ptr<data_output_default_t> data_output_default;
