...
namespace engine
{
	...
	public:
        ...
        {
			logger->log_global_msg(config, "Global setting '#1#' was just changed to '#2#' (triggered by #3# change)!"_u, global, val, source);
        }
        ...
        {
			logger->log_global_msg(config, "App-specific setting '#1#' was just changed to '#2#' (triggered by #3# change)!"_u, app_specific, val, source);
        }
        ...
        {
			logger->log_global_msg(config, "Local setting '#1#' for app '#4#' was just changed to '#2#' (triggered by #3# change)!"_u, local, val, source, app);
        }
