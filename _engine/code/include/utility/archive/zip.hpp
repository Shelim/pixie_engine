...
#include <unzip.h>
#include <iostream>
#include "utility/archive/base.hpp"
...
namespace engine
{
	namespace archive
	{
		...
		class zip_input_t : public input_base_t, public std::enable_shared_from_this<zip_input_t>
		{

		public:

			zip_input_t(std::unique_ptr<data::input_t> input, virtual_path_t path = virtual_path_t(), std::filesystem::file_time_type time_last_mod = std::filesystem::file_time_type::min()) : input_base_t(std::move(input), path, time_last_mod)
			{
				init();
			}
			...
			void construct_all_providers(providers_t * providers) final;

			data::input_t::buffer_t unpack_file(const ustring_t & archive_path) final;
		...
		private:
			...
			static void fill_filefunc(zlib_filefunc64_def_s * filefunc_def, zip_input_t * owner)
			{
				...
				filefunc_def->zopendisk64_file = zip_zopendisk64_fil;
			...
			static void * ZCALLBACK zip_zopendisk64_fil(void * opaque, void * stream, uint32_t number_disk, int mode)
			{
				return nullptr;
			}

			static uint32_t ZCALLBACK zip_zread_file(void * opaque, void * stream, void * buf, uint32_t size)
			{
			...
			static uint32_t ZCALLBACK zip_zwrite_file(void * opaque, void * stream, const void *buf, uint32_t size)
			{
				return 0;
			}
