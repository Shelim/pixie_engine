...
namespace engine
{
	...
	public:
		...
		config_provider_storage_actual_t(std::shared_ptr<messenger_t> messenger, std::unique_ptr<config_provider_base_t::provider_default_t> defaults) : messenger(messenger), defaults(std::move(defaults))
		{
			rescan();
			rdy_vals = vals;
			swap_vals();
...
#include "std/config_std.hpp"
	...
	public:
		...
		config_provider_storage_t(std::unique_ptr<service_t<config_provider_storage_service_t>> service, std::shared_ptr<config_provider_storage_actual_t> actual) : actual(actual), service(std::move(service))
		{
			this->service->start();
		}
