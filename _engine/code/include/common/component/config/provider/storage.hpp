...
#define ENGINE_CONFIG_STD(type_t, name) type_t get_cfg_##name(manifest_app_t::app_t app) const final { std::lock_guard<std::recursive_mutex> guard(ready_mutex); return rdy_vals.val_for_cfg_##name[static_cast<std::underlying_type<manifest_app_t::app_t>::type>(app)]; };
...
#define ENGINE_CONFIG_STD(type_t, name) std::array<type_t, static_cast<std::underlying_type<manifest_app_t::app_t>::type>(manifest_app_t::app_t::count)> val_for_cfg_##name;
#include "std/config_std.hpp"
		};
