...
#define ENGINE_CONFIG_DEF(type_t, name) type_t config_provider_base_t::provider_default_t::get_default_cfg_##name(app_t::kind_t app) const { return configuration->get()->cfg_##name(); }
...
#define ENGINE_CONFIG_DEF(type_t, name) for(auto i = 0; i < value_of(app_t::kind_t::count); i++) { vals.val_for_cfg_##name[i] = config_storage->retrieve(format_string("cfg_#1#_" #name ##_u, manifest_app_t::get_app_name(static_cast<app_t::kind_t>(i))), defaults->get_default_cfg_##name(static_cast<app_t::kind_t>(i))); }
#include "def/config.def"
	}
...
#define ENGINE_CONFIG_DEF(type_t, name) for(auto i = 0; i < value_of(app_t::kind_t::count); i++) {  if(vals.val_for_cfg_##name[i] != old_vals.val_for_cfg_##name[i]) { std::lock_guard<std::recursive_mutex> guard(ready_mutex); rdy_vals.val_for_cfg_##name[i] = vals.val_for_cfg_##name[i]; messenger->post_message(std::make_unique<msg_config_provider_updated_t>(config_t::item_t::cfg_##name)); } }
#include "def/config.def"
	}
}
...
#define ENGINE_CONFIG_DEF(type_t, name) if(item == item_t::cfg_##name) for(auto i = 0; i < value_of(app_t::kind_t::count); i++) {  logger->log_msg(config, "Config initial value for #1# (app: #2#) is #3#"_u, type_to_text(item), manifest_app_t::get_app_name(static_cast<app_t::kind_t>(i)), get_cfg_##name(static_cast<app_t::kind_t>(i))); }
#include "def/config.def"
...
#define ENGINE_CONFIG_DEF(type_t, name) if(item == item_t::cfg_##name) for(auto i = 0; i < value_of(app_t::kind_t::count); i++) {  logger->log_msg(config, "Config changed #1# (app: #2#) -> #3#"_u, type_to_text(item), manifest_app_t::get_app_name(static_cast<app_t::kind_t>(i)), get_cfg_##name(static_cast<app_t::kind_t>(i))); }
#include "def/config.def"
