...
#define ENGINE_CONFIG_STD(type_t, name) for(auto i = 0; i < static_cast<std::underlying_type<manifest_app_t::app_t>::type>(manifest_app_t::app_t::count); i++) {  if(vals.val_for_cfg_##name[i] != old_vals.val_for_cfg_##name[i]) { std::lock_guard<std::recursive_mutex> guard(ready_mutex); rdy_vals.val_for_cfg_##name[i] = vals.val_for_cfg_##name[i]; messenger->post_message(std::make_unique<msg_config_provider_updated_t>(config_t::item_t::cfg_##name)); } }
#include "std/config_std.hpp"
	}
}
