#ifndef ENGINE_COMMON_XML_PIXIE_ARCHIVE_HPP
#define ENGINE_COMMON_XML_PIXIE_ARCHIVE_HPP
#pragma once
...
#define CEREAL_XML_STRING_VALUE "cereal"
	...
	is accomplished through the cereal::cereal::SizeTag object, which will also add an attribute
	to its parent field.
	\ingroup Archives */
	class XMLPixieOutputArchive : public cereal::OutputArchive<XMLPixieOutputArchive>, public cereal::traits::TextArchive
	{
	public:
		/*! @name Common Functionality
		Common use cases for directly interacting wit h an XMLPixieOutputArchive */
		//! @{
		...
		public:
			...
			//! Default options with inclusion of xsl document (give path to it)
			static Options WithXsl(const std::string & path) { return Options(std::numeric_limits<double>::max_digits10, true, false, path); }

			//! Specify specific options for the XMLPixieOutputArchive
			...
			@param xslPath Whether to include xsl document (give path to it) */
		...
		private:
			friend class XMLPixieOutputArchive;
			int itsPrecision;
			bool itsIndent;
			bool itsOutputType;
			std::string itsXSL;
		...
		XMLPixieOutputArchive(std::ostream & stream, Options const & options = Options::Default()) :
			OutputArchive<XMLPixieOutputArchive>(this),
			itsStream(stream),
			itsOutputType(options.itsOutputType),
			itsIndent(options.itsIndent),
			itsXSL(options.itsXSL)
		{
			// rapidxml will delete all allocations when xml_document is cleared
			auto node = itsXML.allocate_node(cereal::rapidxml::node_declaration);
			...
			if (!itsXSL.empty())
			{
				std::string xslVal = "type=\"text/xsl\" href=\"" + itsXSL + "\"\0";
				auto namePtr = itsXML.allocate_string(xslVal.data(), xslVal.length() + 1);

				auto style = itsXML.allocate_node(cereal::rapidxml::node_pi, "xml-stylesheet", namePtr);
				itsXML.append_node(style);
			}

			// allocate root node
			auto root = itsXML.allocate_node(cereal::rapidxml::node_element, xml_detail::CEREAL_XML_STRING);
		...
		~XMLPixieOutputArchive() CEREAL_NOEXCEPT
		{
			const int flags = itsIndent ? 0x0 : cereal::rapidxml::print_no_indenting;
			cereal::rapidxml::print(itsStream, itsXML, flags);
			itsXML.clear();
		}
		...
		void startNode()
		{
			...
			// insert into the XML
			auto node = itsXML.allocate_node(cereal::rapidxml::node_element, namePtr, nullptr, nameString.size());
			...
			void saveValue(T const & value)
		{
			...
			// insert into the XML
			itsNodes.top().node->append_node(itsXML.allocate_node(cereal::rapidxml::node_data, nullptr, dataPtr));
		}
			...
			void insertType()
		{
			...
			// generate a name for this new node
			const auto nameString = cereal::util::demangledName<T>();
	...
	protected:
		...
		struct NodeInfo
		{
			NodeInfo(cereal::rapidxml::xml_node<> * n = nullptr,
				const char * nm = nullptr) :
				node(n),
				name(nm)
			{ }

			cereal::rapidxml::xml_node<> * node; //!< A pointer to this node
	...
	private:
		...
		cereal::rapidxml::xml_document<> itsXML; //!< The XML document
		...
		std::string itsXSL;              //!< Controls whether append XSL path
	}; // XMLPixieOutputArchive
	   ...
	   Input XML should have been produced by the XMLPixieOutputArchive.  Data can
	   ...
	   did not originate from an XMLPixieOutputArchive is not officially supported,
	   but may be possible to use if properly formatted.

	   The XMLPixieInputArchive does not require that nodes are loaded in the same
	   order they were saved by XMLPixieOutputArchive.  Using name value pairs (NVPs),
	   ...
	   \ingroup Archives */
	class XMLPixieInputArchive : public cereal::InputArchive<XMLPixieInputArchive>, public cereal::traits::TextArchive
	{
	public:
		/*! @name Common Functionality
		Common use cases for directly interacting with an XMLPixieInputArchive */
		//! @{
		...
		XMLPixieInputArchive(std::istream & stream) :
			InputArchive<XMLPixieInputArchive>(this),
			itsData(std::istreambuf_iterator<char>(stream), std::istreambuf_iterator<char>())
		{
			try
			{
				...
				itsXML.parse<cereal::rapidxml::parse_trim_whitespace | cereal::rapidxml::parse_no_data_nodes | cereal::rapidxml::parse_declaration_node>(reinterpret_cast<char *>(itsData.data()));
			}
			catch (cereal::rapidxml::parse_error const &)
			{
				...
				throw cereal::Exception("XML Parsing failed - likely due to invalid characters or invalid naming");
			}

			// Parse the root
			auto root = itsXML.first_node(xml_detail::CEREAL_XML_STRING);
			if (root == nullptr)
				throw cereal::Exception("Could not detect 'pixie' root node - likely due to empty or invalid input");
			else
				itsNodes.emplace(root);
		}

		~XMLPixieInputArchive() CEREAL_NOEXCEPT = default;
		...
		void startNode()
		{
			...
			if (expectedName && (next == nullptr || std::strcmp(next->name(), expectedName) != 0))
			{
				...
				if (next == nullptr)
					throw cereal::Exception("XML Parsing failed - requested tag '" + std::string(expectedName) + "' not found");
			}
		...
		template <class T, cereal::traits::EnableIf<std::is_unsigned<T>::value,
			std::is_same<T, bool>::value> = cereal::traits::sfinae> inline
		...
		template <class T, cereal::traits::EnableIf<std::is_integral<T>::value,
			!std::is_same<T, bool>::value,
			sizeof(T) == sizeof(char)> = cereal::traits::sfinae> inline
			void loadValue(T & value)
		{
		...
		template <class T, cereal::traits::EnableIf<std::is_unsigned<T>::value,
			...
			sizeof(T) < sizeof(long long)> = cereal::traits::sfinae> inline
			void loadValue(T & value)
		{
		...
		template <class T, cereal::traits::EnableIf<std::is_signed<T>::value,
			!std::is_same<T, char>::value,
			sizeof(T) <= sizeof(int)> = cereal::traits::sfinae> inline
			void loadValue(T & value)
		{
		...
		template <class T, cereal::traits::EnableIf<std::is_signed<T>::value,
			(sizeof(T) > sizeof(int)),
			sizeof(T) <= sizeof(long)> = cereal::traits::sfinae> inline
			void loadValue(T & value)
		{
	...
	protected:
		...
		static size_t getNumChildren(cereal::rapidxml::xml_node<> * node)
		{
			size_t size = 0;
		...
		struct NodeInfo
		{
			NodeInfo(cereal::rapidxml::xml_node<> * n = nullptr) :
				node(n),
				child(n->first_node()),
				size(XMLPixieInputArchive::getNumChildren(n)),
				name(nullptr)
			{ }
			...
			cereal::rapidxml::xml_node<> * search(const char * searchName)
			{
				if (searchName)
				{
					size_t new_size = XMLPixieInputArchive::getNumChildren(node);
					const size_t name_size = cereal::rapidxml::internal::measure(searchName);

					for (auto new_child = node->first_node(); new_child != nullptr; new_child = new_child->next_sibling())
					{
						if (cereal::rapidxml::internal::compare(new_child->name(), new_child->name_size(), searchName, name_size, true))
						{
							size = new_size;
							child = new_child;
			...
			cereal::rapidxml::xml_node<> * node;  //!< A pointer to this node
			cereal::rapidxml::xml_node<> * child; //!< A pointer to its current child
	...
	template <class T> inline
		void prologue(XMLPixieOutputArchive &, cereal::NameValuePair<T> const &)
	{ }

	//! Prologue for NVPs for XML input archives
	template <class T> inline
		void prologue(XMLPixieInputArchive &, cereal::NameValuePair<T> const &)
	{ }
	...
	/*! NVPs do not start or finish nodes - they just set up the names */
	template <class T> inline
		void epilogue(XMLPixieOutputArchive &, cereal::NameValuePair<T> const &)
	{ }

	//! Epilogue for NVPs for XML input archives
	template <class T> inline
		void epilogue(XMLPixieInputArchive &, cereal::NameValuePair<T> const &)
	{ }
	...
	template <class T> inline
		void prologue(XMLPixieOutputArchive & ar, cereal::SizeTag<T> const &)
	{ }

	template <class T> inline
		void prologue(XMLPixieInputArchive &, cereal::SizeTag<T> const &)
	{ }

	//! Epilogue for SizeTags for XML output archives
	/*! SizeTags do not start or finish nodes */
	template <class T> inline
		void epilogue(XMLPixieOutputArchive &, cereal::SizeTag<T> const &)
	{ }

	template <class T> inline
		void epilogue(XMLPixieInputArchive &, cereal::SizeTag<T> const &)
	{ }
	...
	template <class T, cereal::traits::DisableIf<cereal::traits::has_minimal_base_class_serialization<T, cereal::traits::has_minimal_output_serialization, XMLPixieOutputArchive>::value ||
		cereal::traits::has_minimal_output_serialization<T, XMLPixieOutputArchive>::value> = cereal::traits::sfinae> inline
		void prologue(XMLPixieOutputArchive & ar, T const &)
	{
		ar.startNode();
		ar.insertType<T>();
	}
	...
	template <class T, cereal::traits::DisableIf<cereal::traits::has_minimal_base_class_serialization<T, cereal::traits::has_minimal_input_serialization, XMLPixieInputArchive>::value ||
		cereal::traits::has_minimal_input_serialization<T, XMLPixieInputArchive>::value> = cereal::traits::sfinae> inline
		void prologue(XMLPixieInputArchive & ar, T const &)
	{
		ar.startNode();
	}
	...
	template <class T, cereal::traits::DisableIf<cereal::traits::has_minimal_base_class_serialization<T, cereal::traits::has_minimal_output_serialization, XMLPixieOutputArchive>::value ||
		cereal::traits::has_minimal_output_serialization<T, XMLPixieOutputArchive>::value> = cereal::traits::sfinae> inline
		void epilogue(XMLPixieOutputArchive & ar, T const &)
	{
		ar.finishNode();
	}
	...
	template <class T, cereal::traits::DisableIf<cereal::traits::has_minimal_base_class_serialization<T, cereal::traits::has_minimal_input_serialization, XMLPixieInputArchive>::value ||
		cereal::traits::has_minimal_input_serialization<T, XMLPixieInputArchive>::value> = cereal::traits::sfinae> inline
		void epilogue(XMLPixieInputArchive & ar, T const &)
	{
		ar.finishNode();
	}
	...
	template <class T> inline
		void CEREAL_SAVE_FUNCTION_NAME(XMLPixieOutputArchive & ar, cereal::NameValuePair<T> const & t)
	{
	...
	template <class T> inline
		void CEREAL_LOAD_FUNCTION_NAME(XMLPixieInputArchive & ar, cereal::NameValuePair<T> & t)
	{
		ar.setNextName(t.name);
		ar(t.value);
	}
	...
	template <class T> inline
		void CEREAL_SAVE_FUNCTION_NAME(XMLPixieOutputArchive &, cereal::SizeTag<T> const &)
	{ }

	//! Loading SizeTags from XML
	template <class T> inline
		void CEREAL_LOAD_FUNCTION_NAME(XMLPixieInputArchive & ar, cereal::SizeTag<T> & st)
	{
		ar.loadSize(st.size);
	}
	...
	template <class T, cereal::traits::EnableIf<std::is_arithmetic<T>::value> = cereal::traits::sfinae> inline
		void CEREAL_SAVE_FUNCTION_NAME(XMLPixieOutputArchive & ar, T const & t)
	{
		ar.saveValue(t);
	}
	...
	template <class T, cereal::traits::EnableIf<std::is_arithmetic<T>::value> = cereal::traits::sfinae> inline
		void CEREAL_LOAD_FUNCTION_NAME(XMLPixieInputArchive & ar, T & t)
	{
		ar.loadValue(t);
	}
	...
	template<class CharT, class Traits, class Alloc> inline
		void CEREAL_SAVE_FUNCTION_NAME(XMLPixieOutputArchive & ar, std::basic_string<CharT, Traits, Alloc> const & str)
	{
		ar.saveValue(str);
	}
	...
	template<class CharT, class Traits, class Alloc> inline
		void CEREAL_LOAD_FUNCTION_NAME(XMLPixieInputArchive & ar, std::basic_string<CharT, Traits, Alloc> & str)
	{
		ar.loadValue(str);
	}
}
...
CEREAL_REGISTER_ARCHIVE(cereal::XMLPixieOutputArchive)
CEREAL_REGISTER_ARCHIVE(cereal::XMLPixieInputArchive)

// tie input and output archives together
CEREAL_SETUP_ARCHIVE_TRAITS(cereal::XMLPixieInputArchive, cereal::XMLPixieOutputArchive)
